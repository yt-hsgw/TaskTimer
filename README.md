...
15. タイムラインでは日/週/月の粒度を切り替え、開始予定から期限までを横棒で比較します。日付が片方だけのタスクは1日分、両方ないタスクは `日付未設定` に表示されます。初回実装は参照専用で、バーを選ぶと詳細を開けます。
...
- タイムラインは選択中の対象範囲を維持し、日/週/月の固定期間で開始予定と期限を比較できる。
