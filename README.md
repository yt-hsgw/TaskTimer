...
- 左ナビゲーション: タスクリスト、今日、お気に入り、かんばん、カレンダー、設定を切り替えます。カスタムリストの作成、名称変更、削除もここで行います。`Ctrl+B` で開閉できます。
...
5. タスクを選択すると中央ペイン上に詳細が開き、所属リスト、リスト色、タグ、期限、目標時間、繰り返し、メモ、サブタスク、通知予定、タイマーを確認・編集できます。同じタスクをもう一度選ぶと閉じます。
6. タスクまたはサブタスク行の再生ボタンを押すとカウントダウンが始まり、行内で残り時間を確認できます。開始できる通常タイマーとポモドーロは、合わせてアプリ全体で1件だけです。
7. 作業を中断、再開、終了するときは行内または詳細ペインの操作ボタンを使います。対象で時間を指定していない場合は、設定画面の既定時間（初期値30分）を使用し、0秒でローカル通知します。
