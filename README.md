...
- 左ナビゲーション: タスクリスト、今日、お気に入り、カレンダー、設定を切り替えます。`Ctrl+B` で開閉できます。
...
5. タスクを選択すると右詳細ペインが開き、期限、目標時間、繰り返し、メモ、サブタスク、通知予定、タイマーを確認・編集できます。
...
11. 設定画面で、通知全体のON/OFFと、通知本文の `タイトルのみ` / `汎用メッセージ` を切り替えます。
