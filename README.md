...
- macOS: 利用しているCPUに合うDeveloper ID署名・公証済みDMGをダウンロードします。
...
2. 中央のタスク一覧下部にある追加ボタン、または `Ctrl+N` でタスクを作成します。
3. タスクを選択すると右詳細ペインが開き、開始日、終了日、メモ、サブタスク、通知、タイマーを編集できます。
4. 選択中タスクまたはサブタスクの開始ボタンを押すとタイマーが開始します。開始できるタイマーはアプリ全体で1件だけです。
5. 作業を中断または再開するときは、右詳細ペインのタイマー操作を使います。
6. タスクまたはサブタスクが終わったら円形チェックボックスで完了します。完了済みタスクは完了セクションへ移動します。
7. 週カレンダーで、開始予定、期限、実行中の作業を確認します。
8. 設定画面で、通知本文を `タイトルのみ` または `汎用メッセージ` に切り替えます。
...
MVPの主要機能は実装済みで、初回公開Releaseの準備中です。業務利用前には、Release notes、既知制限、手動確認結果を確認してください。
...
- [v0.1.0 Release notes草案](docs/releases/v0.1.0.md)
...
GitHub Actionsの `リリースビルド` は、`app-v*` タグまたは手動実行でWindows/macOS向けartifactをビルドし、macOS artifactはDeveloper ID署名とApple公証を行ったうえでDraft Releaseへ添付します。
...
macOS署名・公証にはApple Developer Programの証明書とGitHub Secretsが必要です。証明書、Apple ID、App用パスワード、Team IDはリポジトリ、Issue、PR、Release notesに書かないでください。
