...
- 中央ビュー: 選択中のリスト、今日、お気に入りを対象範囲として維持しながら、上部の切り替えでリスト、カンバン、週/日/月カレンダーを表示します。タイムラインは改善完了まで通常導線から一時非表示です。
...
13. カンバンでは、ドラッグ&ドロップでタスクの状態を変更できます。予定未設定カードのドラッグ中は画面下部から `今日`、`明日`、`日時を選択` へ予定を設定でき、列の状態は変わりません。カード右端の三点メニューからも同じ操作をキーボードで選べます。状態は最右端から追加し、列幅全体を操作領域にした上部ハンドルで並べ替えられます。列右端の三点メニューから名称変更、列削除、完了タスクの全件削除、期限・作成日・タイトルによる表示ソートを選べます。完了タスクは元の状態列にある折りたたみセクションへ移動します。
...
- タイムラインは再設計候補としてコードと設計を残していますが、通常導線では一時非表示にする。
- かんばんの各状態列からタスクを追加でき、状態列は最右端から追加、上部ハンドルでの並べ替えができる。列の三点メニューには名称変更、削除、完了タスク全件削除、表示ソートを集約する。タスクはドラッグ&ドロップで移動し、完了は状態とは分離して列ごとの完了セクションに表示する。予定未設定タスクは、状態を変えずに今日、明日、任意日時へ初回予定を割り当てられる。
...
- [AI引き継ぎKDoc風リファレンス](docs/handoff/tasktimer-kdoc.md)
