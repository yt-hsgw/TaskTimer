...
- 左ナビゲーション: タスクリスト、お気に入り、カレンダー、設定を切り替えます。`Ctrl+B` で開閉できます。
- 中央ビュー: 選択中のリスト、完了セクション、タスク追加、週/日/月カレンダー、設定画面を表示します。
...
9. カレンダーで週/日/月を切り替え、開始予定、期限、実行中の作業を確認します。サブタスクには親タスク名が表示され、カレンダー項目を選択すると右詳細ペインで対象を編集できます。
