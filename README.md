...
5. 中央のタスク数表示横にある追加ボタン、または `Ctrl+N` でタスクを作成します。
...
14. カレンダーで週/日/月を切り替え、空きセルをダブルクリックしてタスクを追加します。予定ブロック本体のドラッグで期間を保ったまま移動し、両端のハンドルで開始/終了を調整できます。矢印キーでも移動・調整できます。「他 N 件」から省略された予定を一覧表示でき、サブタスクには親タスク名が表示されます。カレンダー項目を選択すると詳細で対象を編集できます。
