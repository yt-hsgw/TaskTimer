...
MVPの主要機能は実装済みで、初回公開Releaseの準備中です。v0.1.0はWindows版を先行配布し、macOS版はApple署名・公証準備が完了したReleaseで提供します。業務利用前には、Release notes、既知制限、手動確認結果を確認してください。
...
リリース前には [リリース前チェックリスト](docs/release-checklist.md) を使い、Windowsの手動確認、通知権限、オフライン起動、外部通信なしの方針を確認します。macOS artifactを配布する場合はmacOSの署名・公証・Gatekeeper確認も必須です。Draft Releaseは手動確認が終わるまで公開しません。
