...
10. かんばんでは、未着手、進行中、完了の列でタスクを確認し、カード内のボタンで状態を変更できます。
11. カレンダーで週/日/月を切り替え、開始予定、期限、実行中の作業を確認します。サブタスクには親タスク名が表示され、カレンダー項目を選択すると右詳細ペインで対象を編集できます。
12. 設定画面で、通知全体のON/OFF、通知本文の `タイトルのみ` / `汎用メッセージ`、通知失敗履歴と再試行結果を確認します。
13. 設定画面のデータ管理から、SQLiteバックアップ作成、SQLiteバックアップからの復元、JSON/CSVエクスポートを実行できます。
...
- かんばんは未着手、進行中、完了の3列で表示し、クリック操作で状態を変更できる。
