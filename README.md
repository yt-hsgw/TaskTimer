...
13. カンバンでは、ドラッグ&ドロップでタスクの状態を変更できます。状態は最右端から追加し、列上部のハンドルで並べ替えられます。名称変更、削除も行え、完了タスクは元の状態列にある折りたたみセクションへ移動します。ドラッグ操作はキーボードでも利用できます。
...
- かんばんの各状態列からタスクを追加でき、状態列は最右端から追加、名称変更、上部ハンドルでの並べ替え、削除ができる。タスクはドラッグ&ドロップで移動し、完了は状態とは分離して列ごとの完了セクションに表示する。
