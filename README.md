...
- 左ナビゲーション: タスクリスト、今日、お気に入り、かんばん、カレンダー、ポモドーロ、設定を切り替えます。リストの作成、名称、色、削除もここで管理します。タグの付与と管理はタスク詳細へ集約しています。`Ctrl+B` で開閉できます。
- 中央ビュー: 選択中のリスト、今日、完了セクション、タスク追加、かんばん、週/日/月カレンダー、設定画面を表示します。
...
1. 左ペインでタスクリスト、カレンダー、ポモドーロ、設定を切り替えます。
2. 左ペインは `Ctrl+B` で開閉できます。
3. 左ペインの `今日` では、今日が期限のタスクと、今日が期限のサブタスクを持つ親タスクを確認できます。
4. 中央のタスク数表示横にある追加ボタン、または `Ctrl+N` でタスクを作成します。
5. タスクを選択すると中央ペイン上に詳細が開き、所属リスト、タグ、期限、目標時間、繰り返し、メモ、サブタスクを確認・編集できます。同じタスクをもう一度選ぶと閉じます。リスト色は左ペインのリスト編集から変更します。
6. タスクまたはサブタスク行の再生ボタンを押すとカウントダウンが始まり、行内で残り時間を確認できます。開始できる通常タイマーとポモドーロは、合わせてアプリ全体で1件だけです。
7. 作業を中断、再開、終了するときはタスクまたはサブタスク行内の操作ボタンを使います。対象で時間を指定していない場合は、設定画面の既定時間（初期値30分）を使用し、0秒でローカル通知します。
8. 左ペインのポモドーロでは、タスクを選ばずに集中と休憩のサイクルを開始できます。独立ポモドーロの作業時間はタスク別タイマー履歴へ加算されません。
9. タスクまたはサブタスクが終わったら円形チェックボックスで完了します。完了済みタスクは完了セクションへ移動します。
10. 重要なタスクは星ボタンでお気に入りに追加し、左ペインのお気に入りビューで確認します。
11. かんばんでは、ドラッグ&ドロップでタスクの状態を変更できます。状態は追加、名称変更、並べ替え、削除でき、完了タスクは元の状態列にある折りたたみセクションへ移動します。ドラッグ操作はキーボードでも利用できます。
12. カレンダーで週/日/月を切り替え、空きセルをダブルクリックしてタスクを追加します。予定ブロック本体のドラッグで期間を保ったまま移動し、両端のハンドルで開始/終了を調整できます。矢印キーでも移動・調整できます。「他 N 件」から省略された予定を一覧表示でき、サブタスクには親タスク名が表示されます。カレンダー項目を選択すると詳細で対象を編集できます。
13. 設定画面で、通知全体のON/OFF、通知本文の `タイトルのみ` / `汎用メッセージ`、通知の再試行を設定します。
14. 設定画面のエクスポートからJSON/CSVを作成できます。
...
- かんばんの状態列は追加、名称変更、並べ替え、削除でき、タスクをドラッグ&ドロップで移動できる。完了は状態とは分離して列ごとの完了セクションに表示する。
