...
6. Windows実機確認が未完了のまま一時公開する場合は、Release notesに未確認範囲を明記し、pre-releaseとして公開する。
...
12. Windowsコード署名未設定によるOS警告の可能性を既知制限に記載する。
13. 未解決のDependabot alertがある場合は、影響範囲、配布対象、追跡Issueを既知制限に記載する。
