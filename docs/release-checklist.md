...
7. `docs/releases/<version>.md` の草案をもとに、Release notesへ変更点、既知制限、手動確認結果、外部通信なしの方針を記載する。
8. Windowsコード署名未設定によるOS警告の可能性を既知制限に記載する。
9. 未解決のDependabot alertがある場合は、影響範囲、配布対象、追跡Issueを既知制限に記載する。
10. Draft Releaseを公開する。
