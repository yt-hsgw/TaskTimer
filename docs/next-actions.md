...
1. #117 起動・復帰・設定変更時のOS通知再同期を実装する。古いローカルタイマーを残さず、期限到来通知と未来通知予約を同じ入口で再評価する。
2. #115 通知OS登録状態のRepository境界とDB状態を追加する。ネイティブOS登録を採用する場合に備え、通知意図とOS登録状態を混同しない境界を作る。
3. #118 Windows/macOSネイティブ将来通知adapterの実現性を検証する。アプリ完全終了中の通知保証は、この検証後に実装可否を判断する。
4. #72 大量データで一覧とカレンダー表示を検証する。Windows runnerの標準Read Model計測は完了済みのため、Windows実機GUIで主要ビューの標準データ操作結果を追記する。
5. #58 OSスリープ・復帰時のタイマーと通知の実機確認結果を記録する。
6. #22 Tauri経由のglib advisoryを追跡する。
7. #24 macOS署名と公証を設定する。現ラベルはP1だが、Windows優先運用ではApple署名・公証準備ができるまで後回しにする。
