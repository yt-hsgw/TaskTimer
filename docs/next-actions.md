...
1. #80 タスクにタグを付けられるようにする。
2. #81 かんばん形式の画面を追加する。
3. #82 カレンダーからタスクを追加できるようにする。
4. #83 カレンダー上でタスク期間を調整できるようにする。
5. #84 カレンダー上のタスク色を変更できるようにする。
6. #58 OSスリープ・復帰時のタイマーと通知の実機確認結果を記録する。
7. #72 大量データで一覧とカレンダー表示を検証する。Windows実機での標準データ計測結果を追記する。
8. #51 OSへの将来時刻スケジューリング方式を設計する。
9. #22 Tauri経由のglib advisoryを追跡する。
10. #24 macOS署名と公証を設定する。現ラベルはP1だが、Windows優先運用ではApple署名・公証準備ができるまで後回しにする。
