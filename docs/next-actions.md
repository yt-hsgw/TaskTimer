...
1. #115 通知OS登録状態のRepository境界とDB状態を追加する。ネイティブOS登録を採用する場合に備え、通知意図とOS登録状態を混同しない境界を作る。
2. #118 Windows/macOSネイティブ将来通知adapterの実現性を検証する。アプリ完全終了中の通知保証は、この検証後に実装可否を判断する。
3. #72 大量データで一覧とカレンダー表示を検証する。Windows runnerの標準Read Model計測は完了済みのため、Windows実機GUIで主要ビューの標準データ操作結果を追記する。
4. #58 OSスリープ・復帰時のタイマーと通知の実機確認結果を記録する。
