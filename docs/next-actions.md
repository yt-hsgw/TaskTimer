...
- #126: かんばんをドラッグ操作とカスタム状態へ拡張する。
- #127: カレンダー項目を期間表示し、端のドラッグで調整する。
...
1. #126 かんばんの進行状態と完了ライフサイクルを分離し、カード/列のDnDとカスタム状態を実装する。
2. #127 カレンダーの開始/終了期間モデルを確定し、期間表示と端リサイズを実装する。
3. #72 大量データで一覧とカレンダー表示を検証する。Windows runnerの標準Read Model計測は完了済みのため、Windows実機GUIで主要ビューの標準データ操作結果を追記する。
4. #58 OSスリープ・復帰時のタイマーと通知の実機確認結果を記録する。#123 のWindowsネイティブ将来通知PoCも、インストール済みアプリでアプリ完全終了中の発火可否とアンインストール後の予約通知残存可否をこの実機確認結果に含める。
