...
1. #72 大量データで一覧とカレンダー表示を検証する。Windows runnerの標準Read Model計測は完了済みのため、Windows実機GUIで主要ビューの標準データ操作結果を追記する。
2. #58 OSスリープ・復帰時のタイマーと通知の実機確認結果を記録する。#123 のWindowsネイティブ将来通知PoCも、インストール済みアプリでアプリ完全終了中の発火可否とアンインストール後の予約通知残存可否をこの実機確認結果に含める。
3. #22 Tauri経由のglib advisoryを追跡する。
4. #24 macOS署名と公証を設定する。現ラベルはP1だが、Windows優先運用ではApple署名・公証準備ができるまで後回しにする。
