...
1. #72 大量データで一覧とカレンダー表示を検証する。Windows runnerへPresentation描画計測を追加し、Windows実機GUI結果を追記する。
2. #131 タスク一覧の200件上限をページングで解消する。#72で検出したデータ到達性の問題をRepository/Application境界から修正する。
3. #58 OSスリープ・復帰時のタイマーと通知の実機確認結果を記録する。#123 のWindowsネイティブ将来通知PoCも、インストール済みアプリでアプリ完全終了中の発火可否とアンインストール後の予約通知残存可否をこの実機確認結果に含める。
4. #22 Tauri経由のglib advisoryを追跡する。
5. #24 macOS署名と公証を設定する。現ラベルはP1だが、Windows優先運用ではApple署名・公証準備ができるまで後回しにする。
