...
- 既知制限として、Windowsコード署名未設定、Linux未配布、glib advisory追跡、macOS署名・公証の実機確認待ちを明記する。
...
Draft Release公開前に手動確認または署名・公証が失敗した場合は、公開せずRelease Issueへ結果を記録する。
...
- v0.1.0の公開を署名・公証確認後に限定すると公開は遅れるが、Gatekeeper警告により外部利用者が起動できない配布事故を避けやすい。
...
- 署名・公証未確認のDMGを公開して、Gatekeeper警告で利用者が起動できない。
