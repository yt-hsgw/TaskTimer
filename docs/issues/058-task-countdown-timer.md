...
- `timer_sessions.accumulated_paused_seconds` は派生値としてRepositoryで算出し、永続列を増やさない。表示側は終了済み休止区間と現在の休止開始時刻から残り時間を求める。
- 移行前から実行中の通常タイマーとポモドーロ用タイマーは `target_seconds = null` の互換セッションとして残し、自動完了の対象にしない。

## トランザクション境界

`StartTaskCountdown` が対象、単一アクティブ制約、有効秒数を検証し、開始時スナップショットを1トランザクションで作成する。`CompleteExpiredTaskCountdown` は期限到達を再検証してセッションを完了し、DBコミット後に通知する。完了を取得できる呼び出しは1回だけとし、通知成功後に `completion_notified_at` を記録する。
...
sequenceDiagram
  ...
  UC->>OS: privacy settingに従い通知
  OS-->>UC: success / failure
  UC->>DB: success時だけ通知済み時刻を保存
  UC-->>UI: completed session and notification summary
```
...
- DBコミット直後からOS通知送信までにプロセスが終了した場合は通知を失う可能性がある。重複通知を避けることを優先し、完了済みセッションの無制限な再送キューは設けない。
