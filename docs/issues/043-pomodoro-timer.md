...
> 2026-07-19以降、新規開始は [057 独立ポモドーロ](057-standalone-pomodoro.md) の仕様へ移行した。本書のタスク連携開始は既存履歴の互換仕様としてのみ参照する。
...
- 当初は設定画面でポモドーロ既定値を変更できる方針だったが、現行仕様では独立ポモドーロ画面へ移管する。
...
2026-07-17時点で、初回実装スライス、フェーズ遷移スライス、詳細ペイン操作UIスライス、期限到達同期スライスとして以下を追加した。

- `PomodoroPhase`、`PomodoroStatus`、設定値検証をドメイン層へ追加。
- `PomodoroRepository`、`GetPomodoroSettings`、`UpdatePomodoroSettings`、`GetActivePomodoro`、`StartPomodoro`、`PausePomodoro`、`ResumePomodoro`、`CompletePomodoroWorkPhase`、`StartPomodoroBreak`、`SkipPomodoroBreak`、`CompletePomodoroBreak`、`CancelPomodoro` をApplication境界へ追加。
...
- `CompletePomodoroWorkPhase` は作業フェーズ用 `timer_sessions` を停止し、休憩開始前の完了済み作業フェーズとして `pomodoro_sessions` を確定する。
- `StartPomodoroBreak` は完了済み作業フェーズの `cycle_count` と設定値から短休憩/長休憩を判定し、休憩フェーズを開始する。
- `SkipPomodoroBreak` は開始中の休憩をキャンセルし、次の作業フェーズ用 `timer_sessions` と `pomodoro_sessions` を同一トランザクションで作成する。
- `CompletePomodoroBreak`、`CancelPomodoro` は休憩/作業フェーズを正しく終端し、作業フェーズでは通常タイマー履歴も同時に確定する。
- 専用 `PausePomodoro` / `ResumePomodoro` は、作業フェーズでは `timer_pauses`、休憩フェーズでは `pomodoro_sessions.paused_total_seconds` を正として扱う。
...
- ポモドーロ開始、作業一時停止/再開/完了、短休憩/長休憩判定、休憩一時停止/再開/完了、休憩スキップ、キャンセルのRustテストを追加。
- 当初実装では設定画面で作業時間、短い休憩、長い休憩、長い休憩までの作業回数を分単位で編集できるUIを追加した。現行仕様では独立ポモドーロ画面へ移管済み。
- 右詳細ペインのタイマーセクションから、対象タスク/サブタスクのポモドーロ開始、一時停止、再開、作業完了、休憩開始、次の作業開始、休憩完了、休憩スキップ、終了を操作できる。
- ポモドーロ中は右詳細ペインに現在フェーズ、残り時間、状態、セット数、フェーズ予定時間を表示する。
- `SyncExpiredPomodoro` を追加し、アプリ起動中、復帰時、表示更新時に期限到達した作業/休憩フェーズを予定終了時刻で確定できる。
- ポモドーロ作業終了/休憩終了時は、通知全体ON/OFFと通知表示モードに従ってローカル通知を送る。
- `auto_start_break` / `auto_start_next_work` が有効な場合、期限到達同期時に次フェーズを自動開始する。対象タスク/サブタスクが完了済みまたはアーカイブ済みの場合、次作業は開始せず期限到達した休憩だけを確定する。
- 大量データ検証DBにアクティブポモドーロ1件を含め、`active_pomodoro_lookup` をRead Model計測対象へ追加した。

未実装:

- OSへの将来時刻通知の完全予約。これは #51 で扱う。
...
- `CompletePomodoroWorkPhase` は作業フェーズの `timer_sessions.elapsed_seconds` 確定、`pomodoro_sessions.status = completed`、`cycle_count` 加算を同一トランザクションで行う。
- `StartPomodoroBreak` は完了済み作業フェーズを検証し、設定値から短休憩/長休憩を判定して休憩フェーズを開始する。
- `SkipPomodoroBreak` は開始中休憩をキャンセルし、次の作業フェーズ用 `timer_sessions` と `pomodoro_sessions` を同一トランザクションで作成する。
- `CompletePomodoroBreak` は休憩フェーズだけを完了する。休憩は作業履歴に含めない。
- `CancelPomodoro` は作業フェーズなら作業タイマーを停止し、休憩フェーズなら休憩フェーズだけをキャンセルする。
- `PausePomodoro`、`ResumePomodoro` は現在フェーズの一時停止情報を更新する。作業フェーズでは既存 `timer_pauses` と矛盾しないように扱う。
- `SyncExpiredPomodoro` は開始中フェーズの予定終了時刻を計算し、期限到達済みの場合だけ作業/休憩フェーズを確定する。作業フェーズでは `timer_sessions.elapsed_seconds` を予定終了時刻で確定する。
- `SyncExpiredPomodoro` は `auto_start_break` / `auto_start_next_work` が有効な場合、次フェーズ作成までを同一トランザクションで行う。次作業の自動開始時は通常タイマーと同じ開始可能状態を検証し、対象が完了済みまたはアーカイブ済みの場合は休憩完了のみをコミットする。
...
- 右詳細ペインのタイマーセクションに、通常タイマー操作とポモドーロ操作を置く。
- ポモドーロ中は現在フェーズ、残り時間、状態、セット数を表示する。
...
- メモ本文は通知へ含めない。
- 将来時刻通知の完全予約は #51 に従う。#107では `SyncExpiredPomodoro` により、アプリ起動中、復帰時、表示更新時の期限到達同期で扱う。
- ポモドーロ終了通知は、DB上のフェーズ確定後に即時送信する。通知失敗はその場のサマリーで返し、既存の期限通知ルール履歴には混ぜない。
...
- 大量データ性能検証でactive pomodoro lookupを計測し、起動時/復帰時の単一アクティブ復元が大量履歴に引きずられないことを確認する。
...
- ポモドーロモードを開始できる。
- 右詳細ペインでポモドーロの開始、一時停止、再開、作業完了、休憩開始、休憩完了、休憩スキップ、終了を操作できる。
...
3. バックアップ、JSON、CSVエクスポートを更新する。完了。
4. 作業完了、休憩開始/完了、休憩スキップ、キャンセル、一時停止/再開のフェーズ遷移Use CaseとRustテストを追加する。完了。
5. ポモドーロ既定値を編集できるUIを追加する。完了。現行仕様では独立ポモドーロ画面へ移管済み。
6. 右詳細ペインに通常/ポモドーロ切替と進行中表示を追加する。完了。
7. 作業終了/休憩終了通知と復帰時再同期を追加する。完了。
8. 自動開始設定 `auto_start_break` / `auto_start_next_work` の実行処理を追加する。完了。
9. 大量データ計測を更新する。完了。
...
- #107の実装対象は完了。OSへの将来時刻通知の完全予約はスコープ外として #51 で扱う。
- ポモドーロ履歴一覧などの追加要望が出た場合は、直近N件またはページング前提で別Issueへ分割する。
