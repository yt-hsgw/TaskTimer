...
- 200件上限によるデータ欠落は #131 に分離した。#131ではRepository/ApplicationのカーソルページングとPresentationの追加読み込みで解消する。
- Windows実機のTauri/WebView2確認は、性能Issueの完了条件から分離し、リリース前の手動ゲートとして残す。

## データ境界

- `smoke`: 50タスク、各4サブタスク、4リスト。
- `standard`: #131以前は一覧集計5,000件、描画200タスク。#131以後は401件をページ対象とし、初期200件と追加200件を分けて描画する。SQLite側は5,000件を別計測する。
...
- #131のページング後も、5,000件を一度にDOMへ描画する構成ではない。目的のタスクへ素早く到達する検索UIは別の改善余地として残る。
