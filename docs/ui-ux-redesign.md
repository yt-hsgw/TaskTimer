...
- 右詳細ペインで、開始日通知と終了日通知を個別にON/OFFできる。
- 個別OFFは日付を残したまま `notification_rules.enabled = false` として保存する。
- 日付が削除された通知ルールはソフト削除し、個別OFFとは区別する。
- 通知本文の表示タイプは設定ビューで管理し、右ペインでは通知予定の有効/無効を扱う。
...
| 通知個別設定 | 既存 `notification_rules.enabled` | 詳細ペインで個別ON/OFFを扱う。 |
