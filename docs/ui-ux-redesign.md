...
- `週`、`日`、`月` を切り替えられる。
- 週/日表示は、日付のみ行と9:00から18:00の時間軸で開始予定、期限、実行中タイマーを表示する。
...
- 週/日/月の表示切替ができる。
- 日付のみの開始予定/期限は時間指定と誤認しないよう、時間軸とは別の「日付のみ」行へ表示する。
- 実行中タイマーは開始時刻を表示し、9:00から18:00の時間軸に配置する。時間外の場合は最寄りの端の行へ寄せ、実際の時刻テキストを併記する。
