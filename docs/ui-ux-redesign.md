...
- 最下部に設定画面への導線を置く。
- `Ctrl+B` で開閉できる。
- 開閉ボタンは画面上部の `TaskTimer` 左側に枠なしのアイコンとして配置する。
...
- タグの作成、名称変更、削除は親タスク詳細へ集約し、左ペインには表示しない。
- リスト色は識別用スウォッチとして表示し、変更操作は左ペインのリスト編集へ集約する。
- アプリ名は画面全体の主見出しへ集約し、左ペイン内には `TaskTimer` と `ローカルタスク` を重複表示しない。
- 左ペインと右詳細ペインは、中央ビュー切替バーを外側に置かず、同じ作業領域の高さで揃える。
- 新規リスト作成は保存ボタンを置かず、入力欄のみを表示する。Enterまたは編集領域外へのフォーカス移動で保存し、空白または空白のみなら作成せずに入力状態を閉じる。Escapeでは作成せず閉じる。
- リストの編集と削除は件数右側の縦三点メニューへ集約し、メニュー開閉でリスト行の高さを変えない。既定リストは編集だけを表示する。
- リスト編集には保存・取消ボタンを置かず、編集領域外へのフォーカス移動で保存する。空白の名称は編集前の名称へ戻し、クリック先のフォーカスを奪わない。
- 今日とお気に入りをリスト一覧より上に置く。リスト一覧は展開可能にし、追加操作は一覧の下部に置く。
- かんばん、カレンダーは中央ペインの表示切り替えとして扱う。タイムラインは改善完了まで通常導線から一時非表示にする。
...
- 今日ビューでも追加ボタンを表示し、開始予定日を今日にした通常作成プリセットで共通ダイアログを開く。
- タスク作成後は右ペインを自動で開かない。
...
- 状態列の並べ替えハンドルは列カード最上部中央に置く。状態追加は列一覧の最右端に独立した追加スロットとして置き、並べ替え対象やタスクのドロップ先にはしない。
- 状態列の並べ替えハンドルは短い横バーを中央表示し、列幅全体をPointer/Keyboard操作領域にする。
- 状態列見出し右端の縦三点メニューへ状態名編集、状態削除、確認付きの完了タスク全件削除、列内ソートを集約する。ソートは既定順、期限が近い順、作成日の新しい順、タイトル順の表示切り替えとし、DB順序は更新しない。
- 各状態列の未完了一覧上部にタスク追加を置き、共通作成ダイアログで保存したタスクはその列へ同一トランザクションで所属させる。
...
- お気に入り。
- タイトルは表示専用の見た目を基本にし、クリックで入力へ切り替え、フォーカスアウトで保存する。空白なら保存せず編集前へ戻す。
- タグを表示する。
- 親タスク詳細ではチップ末尾の追加操作から、既存タグの候補表示、既存タグの付与、新規タグ作成 + 付与を同じ入力導線で行う。
- タグ追加入力はEnterまたはフォーカスアウトで保存する。空白の場合は追加しない。
- 設定済みタグの名前部分を押すとタグ名を編集でき、Enterまたはフォーカスアウトで保存する。タグ名変更は同じタグを使う他タスクにも反映される。
- タグ入力またはタグ名編集中は、枠ではなく下線だけで編集状態を示す。
- タグ名編集中は `タグを追加` 導線を非表示にし、編集中の対象に集中できるようにする。
- タグの解除は各チップ内の削除アイコンで行う。タグ自体の削除は通常の詳細編集からは外し、必要時は専用のタグ管理UIとして扱う。
- サブタスク詳細では親タスクのタグを継承表示し、直接タグ編集はしない。
- 親タスク詳細では現在の所属リストをクリックして候補ポップアップを開き、所属リスト色とは独立したタスク表示色を変更できる。
- 所属リスト候補にはリスト色とリスト名を表示し、選択時に保存する。Escapeまたはフォーカスアウトでは変更せず閉じる。
- 既定の `タスク` は全タスクを表示するスマート範囲として扱い、タスク詳細の所属リスト候補には表示しない。既存データ互換のため内部IDは維持する。
- 初期表示は現在情報の参照を主にし、タイトル下のサマリーカードは表示しない。
...
- タイマー目標時間は、ネイティブselectではなく既存のリスト選択UIに合わせたボタン型ドロップダウンでプリセットを選択できる。
- 目標時間と繰り返しは概要表示をクリックするとポップアップで編集でき、Enterまたは領域外クリックで保存して閉じる。保存ボタンは置かない。
- 繰り返しは目標時間ドロップダウンの右に並ぶアイコンボタンから小ポップアップを開き、頻度と間隔のみを設定する。
- 繰り返し小ポップアップは領域外クリックで閉じ、目標時間/繰り返し全体のポップアップは領域外クリックで保存して閉じる。
- 目標時間/繰り返しポップアップはEscapeで保存せず閉じ、現在値へ戻す。
- 繰り返しは通常無効の最小表示にし、有効時だけ頻度と間隔を展開する。
- 繰り返し有効化時に期限が未設定の場合は、今日の期限を補完して基準日不足を避ける。
- セクション折りたたみは一旦廃止し、クリックした箇所だけ局所編集へ切り替える。
- 右ペインの局所編集は同時に1つだけ開く。所属リスト、タグ追加、期限、目標時間/繰り返し、サブタスク追加など別の設定値入力を開いた時は、既存の選択状態を閉じる。
- 通知予定の詳細表示は一旦外し、通知全体設定は設定画面へ寄せる。
- メモ。
- メモは詳細の参照表示でも確認でき、クリックでtextareaへ切り替え、フォーカスアウトで保存する。
...
- タスク行からのサブタスク追加は通常タスク追加と同じダイアログを使い、親タスク名を参照表示する。
- 親タスク詳細内のサブタスク追加ボタンは導線として維持し、入力は共通タスク作成ダイアログのサブタスクモードで行う。ダイアログ内では親タスク名を参照表示する。
- 削除は詳細ペイン最下部に配置する。
...
- 左ペインは今日、お気に入り、展開可能なリスト、設定の順に表示する。タグ、カンバン、カレンダー、ポモドーロは置かない。
- タグの付与、解除、管理はタスク詳細へ残す。タグ別ビューは一旦UIから外すが、保存済みタグデータは削除しない。
- 中央ペイン枠内のヘッダーに `リスト`、`カンバン`、`カレンダー` のセグメント切り替えを置く。選択中のリストまたはスマート範囲を維持したまま表示形式だけを切り替える。
- タイムラインは改善完了まで通常導線から一時非表示にする。保存済み設定がタイムラインの場合はリストへ戻す。
- 画面上部は左側に開閉アイコンと `TaskTimer`、右側に中央ペイン幅へ合わせた検索入力とポモドーロアイコンを置く。ポモドーロアイコンは独立ポモドーロ画面へ遷移し、表示中に再度押すと直前の画面へ戻る。
...
- 親タスクの新規作成は、一覧、カンバン、カレンダーのどこから開始しても共通ダイアログを使う。
- `Ctrl+N` は現在の対象リストを初期値にした共通ダイアログを開く。
- 今日スコープでは、最後に選択した実リストと今日の開始予定日を初期値にする。今日という永続リストは作らない。
...
- タスク詳細内のサブタスク追加は作業関係を見ながら入力するため、詳細コンテキスト内に維持する。タスク行からのサブタスク追加は通常タスク追加ダイアログと共通化する。

### タスク詳細と色

- タスク詳細から `タイマー` と `通知` の折りたたみセクションを外す。目標時間と繰り返しは同じ局所編集領域で扱い、通常タイマーの開始・一時停止・再開・終了はタスク行とサブタスク行で扱う。通知の全体設定は設定画面で扱う。
- タスク詳細では所属リストとタスク表示色を別フィールドにする。リスト色の変更は左ペインのリスト編集へ移す。
- リスト色とタスク色はいずれも許可済み色トークンだけを保存する。タスク色が未設定の場合は所属リスト色を表示色として使う。
- タスク表示色は「未設定」と6色のスウォッチから選ぶ。未設定時は所属リスト色を実効色として使う。サブタスク詳細では編集せず、親タスクの実効色を参照する。
- カレンダー項目は左端のアクセントに所属リスト色、本文に実効タスク色を使う。タスク色未設定時は両方とも所属リスト色になる。
- かんばん状態名の編集はEnterに加えてフォーカスが外れた時も保存する。空文字は保存せず元の名前へ戻す。

### ポモドーロ

- ポモドーロ画面は独立した集中画面として扱い、作業、短い休憩、長い休憩、長い休憩までの作業回数、自動開始設定を同じ画面で編集できる。
- 設定画面からポモドーロ設定セクションを外し、設定画面は通知、通常タスクタイマー、エクスポートの運用設定に絞る。
- ポモドーロアイコンはトグル導線とし、ポモドーロ表示中の押下では直前の `AppView` へ戻る。
- 作業フェーズの手動完了では、Application Use CaseがDB更新を先に確定し、その後に既存のローカル通知adapterを副作用として呼ぶ。通知全体OFFまたは通知失敗は状態更新を巻き戻さない。

### タイムライン

- タイムラインは開始予定と期限をタスク単位で横比較する読み取り専用ビューとする。時刻配置と予定操作を行うカレンダーとは責務を分ける。
- 日は14日、週は月曜始まりの12週、月は12か月を固定窓として表示し、前後ボタンで同じ単位数を移動する。
- 左側にタスク名と状態、右側に期間軸を表示する。タスク名列は固定し、期間軸だけ横スクロールできる。
- 開始予定から期限までを1本のバーで表示する。片方だけ設定されている場合はその日を1日バーで示し、両方がない場合は下部の展開可能な「日付未設定」へ移す。
- 表示窓をまたぐバーは端で切り、継続方向を示す。今日の位置には色以外でも識別できる線とラベルを表示する。
- バー本文は実効タスク色、左端は所属リスト色を使う。完了済みタスクはタイトルの取り消し線と低い不透明度を併用する。
- バーと日付未設定行はボタンとしてキーボード操作でき、選択時だけ既存タスク詳細を開く。粒度変更、前後移動、スクロールでは詳細を開かない。
- 既存 `ListTaskPage` の200件ページングと追加読み込みを使い、画面全体ではなくタイムライン内部をスクロールする。
...
- 今日ビューでは、開始予定日または期限日が今日のタスクを表示する。
- 開始予定日または期限日が今日のサブタスクを持つ親タスクも表示する。
- 今日ビューの追加ボタンから作る親タスクは、最後に選択した実リストへ所属し、開始予定日を今日にする。
...
- タスク選択時だけ右ペインが開く。
- 右ペインでサブタスク追加、期限、タイマー目標時間、メモ、削除ができる。
- ポモドーロは画面上部のポモドーロ導線から開く専用画面で操作し、作業/短休憩/長休憩を通常タイマーと誤認されない色とラベルで表示する。
- 右ペインは折りたたみセクションを使わず、クリックした項目だけ局所編集へ切り替える。
