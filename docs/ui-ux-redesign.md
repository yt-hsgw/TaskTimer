...
- 目標タイマー時間設定。
- タイマー開始、一時停止、再開、終了。
- ポモドーロの開始、一時停止、再開、作業完了、休憩開始、休憩完了、休憩スキップ、終了。
- ポモドーロ中の作業、短休憩、長休憩、残り時間、セット数。
- タスク削除。
...
- タグはリストとは別セクションに表示し、作成、名称変更、削除ができる。
- タグ削除時はタスクを削除しないことを確認文に表示する。
...
- `週`、`日`、`月` を切り替えられる。
- ツールバーの追加ボタン、日付セル、時間帯セルからタスク作成を開始できる。
- 作成フォームではタイトル、所属リスト、期限日、期限時刻、メモを入力できる。
- 週/日表示の時間帯セルから作成する場合は、その日付と時間を期限の初期値にする。
- 終日行と月表示の日付セルから作成する場合は、日付だけを期限の初期値にする。
...
- 色は `green`、`blue`、`amber`、`rose`、`violet`、`gray` のプリセットから左ペインで選択する。
...
- ポモドーロ自動開始設定は、実行処理が追加されるまで設定ビューに表示しない。
...
- `未着手`、`進行中`、`完了` の3列を表示する。
- タスクカードにはタイトル、期限、タグ、メモプレビュー、サブタスク進捗、タイマー実行中状態を表示する。
- タスクカードを選択すると右詳細ペインを開く。
- MVPではドラッグ&ドロップではなく、カード内の状態変更ボタンで列を移動する。
- 状態変更はApplication Use Caseを経由して保存する。
- 完了済みタスクは `完了` 列に表示し、アーカイブ済みタスクは通常かんばんから除外する。
- 大量タスク向けの仮想スクロールと列カスタマイズはMVP外とする。
...
stateDiagram-v2
  ...
  CalendarView --> CalendarTaskCreate: 追加ボタン / 日付セル / 時間帯セル
  CalendarTaskCreate --> CalendarView: 作成 / キャンセル
...
- 右ペインのタイマーセクションで通常タイマーとポモドーロを操作できる。
- ポモドーロ中は、作業/短休憩/長休憩が通常タイマーと誤認されない色とラベルで表示される。
