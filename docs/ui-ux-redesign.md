...
現状は、タスク作成、タスク詳細、週カレンダー、通知設定を1画面に並べたMVP検証向けUIである。
...
- 現状の週カレンダーを中央ペインへ移管する。
- 左ペインのカレンダーを選択したときに表示する。
- 表示切替UIとして `週`、`日`、`月` を置く。MVP実装は週表示のみとし、日/月は後続Issueで有効化する。
- 開始予定、期限、実行中タイマーを表示する。
- カレンダー項目を選択すると右ペインで該当タスクまたはサブタスクを表示する。
- 週表示は後続Issueで、月曜から日曜を横並びにし、一般的な始業時間からの時間軸が分かる表示へ拡張する。
...
- 折りたたみセクションはデフォルトで閉じる。
- セクションの表示順は、サブタスク、タイマー、通知とする。
- サブタスクセクションでは親タスクとの関係が分かる説明と進捗件数を表示する。
- タイマー目標時間は詳細フォームで設定できる。
- 削除は詳細ペイン最下部に配置する。

Issue #29 のMVPでは、繰り返し設定の永続化とタイマーの一時停止/再開は扱わない。開始日/期限に紐づく通知予定、タイマー開始/終了、タイマー目標時間、削除確認を右ペインへ集約する。

サブタスク:

- タスク詳細内で追加できる。
- サブタスクごとに完了、開始日、期限、タイマー目標時間、タイマー対象を扱える。
- サブタスク行を選択すると、右ペイン内でサブタスク編集状態へ切り替える。
- 親タスク詳細へ戻る導線を持つ。
- Issue #29 のMVPでは、右ペイン内のインライン編集で開始日、期限、目標時間を更新する。サブタスク単体の選択状態と戻る導線は、サブタスク詳細が肥大化した時点で追加する。
...
- 開始日がある場合は開始予定通知、期限がある場合は期限通知を表示する。
- 日付更新時に通知ルールを同一トランザクションで同期する。
- 日付が削除された通知ルールは無効化する。
- 通知本文の表示タイプは設定ビューで管理し、右ペインでは通知予定の有無を扱う。
...
- 週/日/月の表示切替UIは先に配置するが、日/月ビューの実表示はMVP外とする。
- 時間軸付き週表示は、一般的な始業時間から開始する日別カラムとして後続Issueで実装する。
...
| 通知個別設定 | 既存 `notification_rules` 拡張 | 詳細ペインで個別ON/OFFを扱う。 |
...
- タスク選択時だけ右ペインが開く。
- 右ペインでサブタスク追加、期限、通知、タイマー目標時間、タイマー操作、削除ができる。
