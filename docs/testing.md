...
- 状態列を指定したタスク作成はその列へ保存され、削除済み状態列を指定した場合はタスクを残さず失敗する。
- 状態列の完了タスク全件削除は対象列だけへ作用し、未完了タスク、他列の完了タスク、失敗時の全データを維持する。
...
- 通常タイマーは開始時点の対象別時間または既定時間をセッションへ保存し、その後の設定変更で終了時刻が変わらない。
- 一時停止時間を除いた残り時間が0秒になったときだけ自動完了し、重複完了しない。
- 通知全体設定がOFFの場合も自動完了するが、OS通知は送らない。
...
- UI設定更新で最後のビューに `timeline` を保存し、再取得できる。
...
- 設定画面のエクスポートUIで、JSON/CSVの成功、失敗、キャンセル状態が表示される。
- エクスポート操作中は他のエクスポート操作を再実行できない。
- SQLiteバックアップ/復元Use CaseのテストはUI非表示後も維持する。
...
- タスクの作成、編集、削除。
- サブタスクの作成、編集、削除。
- タスク/サブタスク行でカウントダウンの開始、一時停止、再開、終了ができ、操作で詳細ペインが開かない。
- 実行中の対象行だけが1秒ごとに残り時間を更新し、App Shell全体を再描画しない。
- タイマー開始中にアプリを再起動する。
- カレンダーの週/日/月切替と前後移動。
- タイムラインの日/週/月切替と前後移動で固定長の軸が表示され、切替だけでは詳細を開かない。
- タイムラインで開始予定だけ、期限だけ、両方、日付未設定のタスクが欠落せず、バー選択から詳細を開ける。
- タイムラインの追加読み込み、内部スクロール、固定タスク名列が大量データでも破綻せず、App Shell全体を再読み込みしない。
- カレンダーの空き時間帯をドラッグし、選択した15分単位の期間で作成フォームが開く。
- カレンダーの上部予定行と月表示をドラッグし、選択した複数日の終日予定で作成フォームが開く。
- 日/週カレンダーで完全重複・部分重複する予定が横レーンへ分割され、境界時刻が一致する非重複予定は全幅になる。
- 日/週カレンダーで8:00から22:00までの水平線と日列の垂直線が表示され、時間ラベル・セル・予定の位置が一致する。
- 月表示と日/週の上部予定行で「他 N 件」を開き、全予定の表示、詳細選択、Esc・外側クリックでの終了、起点へのフォーカス復帰を確認する。
