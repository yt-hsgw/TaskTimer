...
- `DeleteCompletedTasksInBoardColumn`
- `UpdateTask`
- `CreateSubtask`
- `UpdateSubtask`
- `StartTimer`
...
| --- | --- |
| CreateTask | タスクを追加する。 |
| CreateTaskInBoardColumn | タスク入力と状態列IDを検証し、状態列の存在確認とタスク追加を同一トランザクションで行う。 |
| DeleteCompletedTasksInBoardColumn | 状態列IDを検証し、その列に所属する完了タスクと配下データを同一トランザクションでソフト削除する。 |
...
GitHub #178では、各状態列からのタスク作成を `CreateTaskInBoardColumn` へ接続する。Applicationはタスク入力と状態列IDを検証し、Infrastructureは有効な状態列の存在確認とタスクINSERTを同一SQLiteトランザクションで行う。`CreateTask` 後に `MoveTaskToBoardColumn` を呼ぶ案は、後段失敗時に別列へタスクが残るため採用しない。

同Issueの列メニューでは `DeleteCompletedTasksInBoardColumn` を破壊的操作の境界にする。Repositoryは対象列の完了タスクIDを取得し、既存のタスクグラフソフト削除を1トランザクションで適用する。列内ソートはPresentationの派生表示であり、Repositoryや `sort_order` を更新しない。
...
GitHub #157では、左ペインが選択する対象範囲と中央ペインの表示形式をPresentationで別stateとして保持する。リスト、今日、お気に入りの各スコープを `ListTaskPage` と `ListCalendarItems` の両方へ渡し、リスト、カンバン、カレンダー、タイムラインを切り替えても対象範囲を維持する。

GitHub #160のタイムラインは新しい永続モデルやRepository Queryを追加せず、`ListTaskPage` のタスクツリーと `TaskRowItem` をPresentationで期間行へ射影する。開始予定を開始、期限を終了として扱い、片側だけの日付は1日バー、両方がない場合は日付未設定行とする。日14日、週12週、月12か月の固定窓と既存の最大200件ページングにより描画量を制限する。読み取り専用のためApplicationの更新トランザクション、カレンダー予定移動Use Case、通知再登録には接続しない。最後の表示形式として `timeline` だけをUI設定の許可値へ追加する。
...
| `TaskPage` | 中央一覧・かんばん・タイムラインのページ境界 | 同一ID集合のタスクツリーと`TaskRowItem`、DB総件数、次カーソル、今日・お気に入り集計。 |
