...
- macOS artifactはDeveloper ID署名とApple公証を行う。詳細は [Issue 014](../issues/014-release-macos-signing-notarization.md) に記録する。
- Windowsコード署名は未設定のため、OS警告の可能性を既知制限として明記する。
...
- Windowsコード署名は未設定のため、Windows SmartScreenで警告が出る可能性がある。
...
- macOS公証失敗またはWindows署名警告を既知制限に書かず、利用者がインストール可否を判断できない。
