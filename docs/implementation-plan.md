...
- 詳細は基本非表示にし、タスク選択時に中央ビュー上のオーバーレイとして開く。開閉で中央ビュー幅を変えない。
...
9. 左ナビの操作整理、詳細へのリスト色/タグ管理集約、カレンダーのダブルクリック追加、設定画面のJSON/CSV限定表示を反映する。完了済み。
10. かんばんのカスタム状態とDnD、カレンダーの期間モデルと端リサイズを実装する。完了済み。

危険ケース:

- UIだけ先に作り、リスト、お気に入り、タイマー一時停止の正がPresentationに寄る。
- 完了済みタスク移動や進捗バーが、DB状態と不一致になる。
- 詳細オーバーレイの削除操作が狭い画面で誤操作される。
...
3. 画面上部導線と中央ペインに独立ポモドーロ画面を追加する。完了。
4. ポモドーロ画面に既定値設定を追加する。完了。
5. 作業終了/休憩終了通知と復帰時再同期を追加する。完了。
